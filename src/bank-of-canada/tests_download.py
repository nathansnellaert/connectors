...
def test_series_catalog_nonempty():
    """The series catalog should hold thousands of rows; a tiny count means the
    listing endpoint changed shape or returned an error envelope."""
    t = load_raw_parquet("bank-of-canada-series")
    assert len(t) > 1000, f"series catalog has only {len(t)} rows"
    assert {"series_id", "label", "description", "link"} <= set(t.column_names)
...
def test_values_batches_present():
    """The firehose writes one parquet per chunk/refresh; none means the
