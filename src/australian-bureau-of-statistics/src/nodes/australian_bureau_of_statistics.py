"""Australian Bureau of Statistics connector — SDMX 2.1 REST (data.api.abs.gov.au).

Mechanism: the public ABS SDMX 2.1 REST surface (chosen mechanism `sdmx_21`,
auth_kind=none since the Nov-2024 redesign removed API keys). Each dataflow is
fetched in full via /rest/data/<dataflow>/all with `format=csv`, which returns
the entire dataflow as one SDMX-CSV document (DATAFLOW, dimension columns,
TIME_PERIOD, OBS_VALUE, attribute columns). One DOWNLOAD_SPEC per dataflow in
the entity union; one SqlNodeSpec per dataflow publishing the long-format
observation table.

Fetch shape: stateless full re-pull (shape 1). The SDMX API has no usable
incremental delta for our whole-dataset snapshot pattern (research handoff:
"No incremental query - full corpus per refresh"), and each dataflow is a
single bounded request that re-pulls in seconds-to-minutes. Revisions and late
corrections are picked up for free. Freshness gating is the maintain step's job.

Raw format: SDMX-CSV saved verbatim as a `.csv` raw asset. Dimension columns
differ per dataflow (CPI has MEASURE/INDEX/REGION/...; ABS_DEM_QIM has
SEX/STATE_DEP/STATE_ARR/...), so there is no single stable parquet schema across
entities. CSV is SQL-readable (read_csv_auto) and preserves the source shape
exactly; the transform casts OBS_VALUE and drops empty observations.
"""
...
class _PermanentFetchError(Exception):
    """A 4xx (except 429) for one dataflow — skip it, don't retry, don't crash."""
...
def _fetch_dataflow_csv(entity_id: str) -> bytes:
    url = f"{_BASE}/data/{entity_id}/all"
    resp = get(
        url,
        params={"format": "csv"},
    ...
    if resp.status_code in (400, 404) or (
        ...
        # Permanent for this dataflow (e.g. retired DSD). Surface as permanent
        # so the caller writes a skip marker instead of retrying.
...
# Reverse-map the spec id back to its source dataflow id. Ids are uppercase
# [A-Z0-9_] (verified against the entity union), so lower()+'_'->'-' is fully
# reversible; building the dict from ENTITY_IDS keeps it exact regardless.
_NODE_TO_ENTITY = {
    f"australian-bureau-of-statistics-{eid.lower().replace('_', '-')}": eid
...
def fetch_one(node_id: str) -> None:
    ...
    entity_id = _NODE_TO_ENTITY[node_id]
    state = load_state(asset)
    if state.get("schema_version") != STATE_VERSION:
        state = {"schema_version": STATE_VERSION}

    try:
        content = _fetch_dataflow_csv(entity_id)
    except _PermanentFetchError as exc:
        print(f"[abs] permanent error, skipping {entity_id}: {exc}", flush=True)
    ...
    skipped = state.get("skipped", {})
    skipped.pop(entity_id, None)  # recovered
    state["skipped"] = skipped
    state["last_run_stats"] = {"bytes": len(content)}
    save_state(asset, state)
...
# One published Delta table per dataflow: the long-format observation table.
# Dimension columns vary per dataflow, so we keep them all via SELECT * and only
# re-type OBS_VALUE to DOUBLE, dropping rows with no numeric observation. A
# dataflow that publishes 0 numeric rows fails its node (the correctness gate).
TRANSFORM_SPECS = [
    SqlNodeSpec(
        id=f"{s.id}-transform",
        deps=[s.id],
