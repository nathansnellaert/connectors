"""Download node for the Australian Bureau of Statistics (ABS).

ABS exposes a single public SDMX 2.1 REST surface at
https://data.api.abs.gov.au/rest (the November 2024 redesign removed the
API-key requirement). Each dataflow is fetched in full via
/rest/data/<dataflow>/all and the API serves SDMX-CSV via the
`Accept: application/vnd.sdmx.data+csv` header / `format=csv` query param.

Shape (e), snapshot-only: our access pattern is whole-dataset snapshots
per refresh — the API has no incremental cursor we use, so each refresh
re-fetches the full dataflow once its on-disk snapshot ages past the
freshness window. Idempotency is enforced by `raw_asset_exists`; state
carries only TTL-bound skipped markers (dataflows that 4xx / return
empty) plus last-run stats.

Memory / disk discipline (this is why this node streams):
ABS dataflows fetched as `/all` snapshots are highly skewed in size —
most are a few hundred KB, but the detailed small-area cross-tabs
(Building Approvals by SA2, Census G-tables by SAL) run to multiple GB
of uncompressed SDMX-CSV. A prior revision buffered each response whole
in RAM (`resp.content`) and wrote it uncompressed, which blew both the
spawn-subprocess RSS ceiling and local disk. This revision streams the
response chunk-by-chunk straight into a gzip-compressed raw file: peak
RAM stays at one chunk, and SDMX-CSV (heavily repeated dimension codes)
compresses ~10x on disk.
"""
...
REST_BASE = "https://data.api.abs.gov.au/rest"
# SDMX-CSV media type — ASCII only (httpx rejects non-ASCII header values).
SDMX_CSV = "application/vnd.sdmx.data+csv"
# Raw files are stored gzip-compressed; SDMX-CSV compresses ~10x.
RAW_EXT = "csv.gz"
# A dataflow that 4xx's or comes back empty is parked for this long before
# we probe it again, so a transient source-side outage self-recovers.
SKIP_TTL_SECONDS = 14 * 86400
SCHEMA_VERSION = 1
# Decoded bytes below this are not a real dataset (header-only / blank
# body) — a genuine ABS dataflow returns far more even with one series.
MIN_MEANINGFUL_BYTES = 128
# Per-request timeouts. `read` is the gap-between-chunks budget, not a
# total cap, so a multi-GB stream is fine as long as data keeps flowing.
_TIMEOUT = httpx.Timeout(connect=15.0, read=300.0, write=30.0, pool=30.0)

# The entity union — every dataflow rank scored at/above the publish
# threshold. Copied verbatim from the entity-union file. Inlined (not
# read from disk) because module-level I/O is disallowed.
ENTITY_IDS = [
    'ABORIGINAL_ID_POP_PROJ', 'ABORIGINAL_POP_PROJ', 'ABS_ANNUAL_ERP_ASGS2021',
    'ABS_ANNUAL_ERP_LGA2024', 'ABS_DEM_QIM', 'ABS_ERP_COB_STATE', 'ABS_FAMILY_PROJ',
    'ABS_HH_PROV', 'ABS_LABOUR_ACCT', 'ABS_LABOUR_ACCT_UNBAL', 'ABS_NOM_VISA_CY',
    'ABS_NOM_VISA_FY', 'ABS_PERSONS_PROJ', 'ABS_REGIONAL_ASGS2021',
    'ABS_REGIONAL_INDIGENOUS_2021', 'ABS_REGIONAL_LGA2021', 'ABS_REGIONAL_MIGRATION',
    'ABS_REGIONAL_REMOTENESS_ASGS2021', 'ABS_SEIFA2021_LGA', 'ABS_SEIFA2021_POA',
    'ABS_SEIFA2021_SA1', 'ABS_SEIFA2021_SA2', 'ABS_SEIFA2021_SAL', 'ABS_SU_TABLE_2024',
    'AG_BROADACRE', 'AG_HORTICULTURE', 'ALC', 'ANA_AGG', 'ANA_EXP', 'ANA_INC', 'ANA_IND_GVA',
    'ANA_SFD', 'ATSI_BIRTHS_SUMM', 'ATSI_FERTILITY', 'AUSTRALIAN_INDUSTRY', 'AWE', 'BA_GCCSA',
    'BA_LGA2025', 'BA_SA2', 'BIRTHS_AGE_FATHER', 'BIRTHS_AGE_MOTHER', 'BIRTHS_MONTH_OCCURRENCE',
    'BIRTHS_MON_OCC_AGE_MOTHER', 'BIRTHS_SUMMARY', 'BOP', 'BOP_GOODS', 'BOP_STATE',
    'BUILDING_ACTIVITY', 'BUSINESS_TURNOVER', 'BWD', 'C21_G01_CED', 'C21_G01_LGA',
    ...
    'C21_T35_LGA', 'C21_T35_SA2', 'CAPEX', 'CAPEX_EST', 'CONFINEMENTS_NUPTIALITY', 'CPI',
    'CPI_M', 'CPI_Q', 'CPI_WEIGHTS', 'CWD', 'DEATHS_AGESPECIFIC_OCCURENCEYEAR',
    ...
    'LF_HOURS', 'LF_UNDER', 'LSTOCK_MEAT', 'LSTOCK_SLAUGHT', 'MERCH_EXP', 'MERCH_IMP',
    'MIN_EXP', 'NIM_CY', 'NIM_FY', 'NOM_CY', 'NOM_FY', 'OAD_COUNTRY', 'OAD_REASON', 'OMAD_VISA',
...
def _is_transient(exc: BaseException) -> bool:
    """Retry network blips and 429/5xx; let everything else propagate."""
...
def _asset_id(entity_id: str) -> str:
    return f"australian-bureau-of-statistics-{entity_id.lower().replace('_', '-')}"
...
def _download_to_raw(entity_id: str, asset: str) -> int:
    """Stream one dataflow's full SDMX-CSV snapshot into a gzip raw file.

    Returns the number of decoded (uncompressed) bytes received. Raises
    httpx.HTTPStatusError on a non-2xx response. The whole stream runs
    inside the retried function, so a transient mid-stream failure simply
    restarts the download — `raw_writer` opens in "wb" mode, so the retry
    overwrites any partial gzip file rather than appending to it.
    """
    url = f"{REST_BASE}/data/{entity_id}/all"
    client = get_client()
    total = 0
    # `client.stream` keeps the body off the heap; httpx transparently
    # negotiates gzip on the wire and yields decoded CSV bytes.
    with client.stream(
        "GET",
        url,
        params={"format": "csv"},
        headers={"Accept": SDMX_CSV},
        timeout=_TIMEOUT,
    ) as resp:
        resp.raise_for_status()
        with raw_writer(asset, RAW_EXT, mode="wb", compression="gzip") as f:
...
def fetch_one(entity_id: str) -> None:
    """Download one ABS dataflow as a full, gzip-compressed SDMX-CSV snapshot."""
    asset = _asset_id(entity_id)
    # ABS releases datasets continuously — individual series update monthly
    # to quarterly (e.g. Labour Force monthly, CPI quarterly) per the ABS
    # release calendar at https://www.abs.gov.au/release-calendar. A 7-day
    # freshness window keeps snapshots current without daily refetches.
    if raw_asset_exists(asset, ext=RAW_EXT, max_age_days=7):
        return

    state = load_state(asset)
    if state.get("schema_version") != SCHEMA_VERSION:
        # Unknown / absent schema version: reset to empty.
        state = {"schema_version": SCHEMA_VERSION, "skipped": {}}

    now = int(time.time())
    # Drop expired skip markers so a recovered source self-heals.
    skipped = {
        eid: meta
        for eid, meta in state.get("skipped", {}).items()
        if meta.get("expires_at", 0) > now
    }
    if entity_id in skipped:
        # Parked by a recent permanent failure; TTL not yet elapsed.
        state["skipped"] = skipped
        save_state(asset, state)
        return

    try:
        total = _download_to_raw(entity_id, asset)
    except httpx.HTTPStatusError as exc:
        code = exc.response.status_code
        # A non-429 4xx is a permanent client error for this entity (the
        # dataflow was retired, or `/all` currently exposes no data and
        # ABS answers 404). Park it with a TTL and move on — one bad
        # entity must not fail the whole step.
        if code != 429 and 400 <= code < 500:
            print(
                f"[abs] permanent HTTP {code} for {entity_id} "
                f"at {exc.request.url} - parking",
                flush=True,
            )
            skipped[entity_id] = {
                "reason": f"HTTP {code}",
                "expires_at": now + SKIP_TTL_SECONDS,
            }
            state["skipped"] = skipped
            save_state(asset, state)
            return
    ...
    # A 200 with an effectively empty body means the dataflow exposes no
    # observations right now — drop the near-empty gzip file and park it.
    if total < MIN_MEANINGFUL_BYTES:
        print(
            f"[abs] empty body for {entity_id} ({total} bytes) - parking",
            flush=True,
        )
        delete_raw_file(asset, RAW_EXT)
        skipped[entity_id] = {
            "reason": f"empty response ({total} bytes)",
            "expires_at": now + SKIP_TTL_SECONDS,
        }
        state["skipped"] = skipped
        save_state(asset, state)
    ...
    # Raw is already on disk (written inside _download_to_raw); persist
    # state after it so a crash costs at most a re-fetch, never a phantom.
    state["skipped"] = skipped
    state["last_run_stats"] = {"decoded_bytes": total}
    save_state(asset, state)
    print(f"[abs] {entity_id}: {total} decoded bytes", flush=True)
